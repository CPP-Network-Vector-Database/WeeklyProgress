...
# --- Sidebar: Reset Graphs Button ---
def reset_graphs_and_metrics():
    # Remove operation_metrics.json
    metrics_file = os.path.join(os.path.dirname(__file__), 'operation_metrics.json')
    if os.path.exists(metrics_file):
        os.remove(metrics_file)
    # Remove log files in weaviate-benchmarking
    log_dir = BASE_DIR
...
if st.sidebar.button("Reset Plots & Analysis Graphs", help="Delete all logs and metrics for a fresh start"):
    reset_graphs_and_metrics()
    st.sidebar.success("All logs, metrics have been reset!")
...
with tabs[0]:
    ...
    if st.button("Ingest Data"):
        ...
        st.info(f"Index Construction Time (to last object searchable): {metrics['index_construction_time']:.2f}s")
